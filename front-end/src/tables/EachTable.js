...
function EachTable({ table }){
    ...
    async function handleFinish(event){
        ...
        if(window.confirm("Is this table ready to seat new guests? This cannot be undone.")){
            try {
                await axios.delete(`http://localhost:5001/tables/${table.table_id}/seat`)
                history.push("/")
        ...
        return (
            <>
                <div className="border border-dark p-3 mb-2">
                    ...
                    {table.reservation_id !== null ? <button data-table-id-finish={table.table_id} onClick={handleFinish}>Finish</button> : ""}
                </div>
            </>
        )
    }
